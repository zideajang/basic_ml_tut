...
从下面两图可得, 上图图中所假设的概率分布求出的似然函数取值显然比下面的小, 因为有很接近于$0$的数值, 这让似然函数乘积的结果变得非常小. 我们可以得出结论, 下图的概率分布参数更符合观测到的数据点的概率分布,而最大似然估计的目的就是找到一个最符合当前数据的分布的参数.
...
A 和 B表示事件，P(A) 是观察到事件 A 的概率，P(B) 是观察到事件 B的概率。P(A|B)是观察到 B 的同时又观察到事件 A 的条件概率。
#### 概率
推动概率论
...
其实似然函数也是条件概率，只是我们观察关注的变量值 x 改变为参数值 $\mu $ 和 $\sigma$
...
$$ L(\mu, \sigma | X) = \sum_{i=1}^N \log P(x_i|\mu,\sigma)$$

为什么要对似然函数取对数?

首先原本的似然函数是很多条件概率的乘积, 我们在找极大值的时候需要求似然函数的导数, 而乘积的导数不方便计算, 取对数可以吧乘除变成加减;
对似然函数取对数, 原本函数的极大值的位置没有改变;
如果观测到的数据点比较多, 原始似然函数的乘积可能非常接近于0, 甚至超出计算机的储存位数限制, 这样就全变成0了, 取对数可以把接近于$0$的数变成很大的负数, 也就是把原本似然函数的取值范围从$0$到$1$扩展到了$-\infty$到$0$, 方便了计算

### MAP(最大后验估计) Maximum a Posterior
$$ X = {x_1, x_2, \dots x_N} x_i \in  $$
假设$P(X,\theta)$
目标：找到一个$\theta$ 拟合 N
选择 $\theta_{map} = arg max_{\theta} P(\theta|X) $
...
$$ \theta_{map} = arg \max \frac{P(y,x|\theta)P(\theta)}{P(y,x)} $$
$$ \theta_{map} = \arg \max  \log P(y,x| \theta) + \log P(\theta) $$

最大似然估计 $ \log P(y,x| \theta) $ 
先验$ \log P(\theta)$
